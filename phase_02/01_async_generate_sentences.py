...
# === Define the logs and intermediate paths relative to this ===
logs_dir = os.path.join(LOG_FOLDER, "logs")
...
class SharedState:
    ...
    async def save_intermediate_results(self):
        ...
        csv_path = os.path.join(base_path, f"{prefix}_sentences_{timestamp}.csv")
...
async def run_aggregation(prompt_file: str, target_count: int = TARGET_COUNT_PER_WORD, 
    ...
    async with get_ollama_client() as client:
        for model in models:
            ...
            model_dir = os.path.join(LOG_FOLDER, model.replace(":", "_"))
